...
export function cheerFallback(b) {
  if (b.chapter_done) return "That was the last fence of the chapter. A new chapter is open.";
  if (b.first_try) return { fix: "You counted the gaps, not the planks. The fence is whole again.",
    share: "Every part got its full share. That is fair sharing." }[b.mode] || "Every part right on the first try. That is careful counting.";
  if (b.fixed_after_count) return "You counted a full part and then fixed the short part. That is how it is done.";
