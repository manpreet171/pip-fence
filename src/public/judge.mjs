...
export function mount(snap) {
  ...
  function update() {
    ...
    document.getElementById("jbody").innerHTML =
      ...
      `<h3>next fence · who chose it</h3><pre>${s.plan ? `chosen by  ${esc(s.plan.source)}${s.plan.reason ? `  (${esc(s.plan.reason)})` : ""}\nnode       ${esc(s.plan.node)}  ${s.plan.valid ? "in code's list" : "code's own pick"}\nwhy        ${esc(s.plan.why || "(no line)")}\nchoices    ${esc(s.plan.choices.join(" "))}` : "<span class=dim>none yet: the plan is made when she taps Next plot</span>"}</pre>` +
      `<h3>worked example · who wrote the moves</h3><pre>${s.show ? `written by ${esc(s.show.source)}${s.show.reason ? `  (${esc(s.show.reason)})` : ""}\nmoves      ${esc(s.show.steps.map(t => t.op + (t.part != null ? " " + t.part : "") + (t.text ? ` "${t.text}"` : "")).join("\n           "))}` : "<span class=dim>none yet: Show me, Pip appears after the second hint</span>"}</pre>` +
