...
| 1:35–1:50 | **The ladder** | `node heritage/evals/classifier_eval.mjs` output scrolling: the count and even ids in the confusion matrix. | "The engine already carries the next rungs: the same board asks how many seeds a pit will hold, then even or odd. The classifier names four more miscounts, one hundred and thirty-nine fixtures green. The screens for those levels are the next build, and I say so rather than show a mock." [classifier_eval, HD-012] |
...
- Start on `2_single`; the miss shot uses a three-seed pit on `3_single` (`?node=3_single`); the ladder shot is the eval output, not the board.
