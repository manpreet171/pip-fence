// judge.mjs — the reviewer's overlay. Loaded only on ?judge=1 or the J key; the child's page never
...
#judge .ev i{font-style:normal;color:#9fdc9f}
...
export function mount(snap) {
  ...
  function update() {
    ...
    const { age, tier, reading_level, ...rest } = p;                            // the only fields allowed a digit
    const clean = !/\d/.test(JSON.stringify(rest));
    const ev = s.events.slice(-10).map(e => { const { t, e: name, ...x } = e;
      return `<div class="ev">${String(t).padStart(6)} ${esc(name)} ${Object.keys(x).length ? j(x) : ""}${name === "turn" ? `  <i>other side: code, p_best = ${s.p_best}</i>` : ""}</div>`; }).join("");
    document.getElementById("jbody").innerHTML =
      ...
      `<h3>other side</h3><pre>code, p_best = ${s.p_best}  ·  one-ply lookahead, no model near a move</pre>` +
      `<h3>mastery · ${GRAPH.length} nodes</h3><pre>${GRAPH.map(g => `${g.node.padEnd(10)} ${s.mastery[g.node] ?? 0}`).join("\n")}</pre>`;
  }
