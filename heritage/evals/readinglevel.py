"""
READING-LEVEL CHECK for buddy hints (the reading level is measured, not asserted).

Scores each template in heritage/data/hints.txt (id<TAB>tier<TAB>text) against heritage/data/wordlist.txt:
...
number-word flag. Inflections -s/-es/-ing/-ed are stripped before lookup (CONCEPT §6).
No LLM involved - pure code. NUMBER_WORDS is the same 36-word list as gate() in buddy.mjs.

Run:  python heritage/evals/readinglevel.py            (table for every template)
      python heritage/evals/readinglevel.py hints.txt  (same format, another file)
      python heritage/evals/readinglevel.py --assert   (build gate: <=2 sentences, 0 digits/number words,
...
NUMBER_WORDS = {"zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten",
    ...
    "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth", "ninth", "tenth", "eleventh", "twelfth"}   # ordinals leak the group count; "first" allowed (HD-008)
assert len(NUMBER_WORDS) == 36

def load_list(path="heritage/data/wordlist.txt"):
...
def load_hints(path="heritage/data/hints.txt"):
    rows = []
    for raw in io.open(path, encoding="utf-8"):
...
if __name__ == "__main__":
    ...
    if "--assert" in sys.argv:            # build-time gate check on the committed templates (CONCEPT §6)
        bad = 0
        ...
        if len(hints) != 39 or len(ids) != 13 or any((m, t) not in {(a, b) for a, b, *_ in hints} for m in ids for t in (1, 2, 3)):
            print(f"FAIL: expected 13 ids x 3 tiers, got {len(hints)} lines over {len(ids)} ids"); bad += 1
