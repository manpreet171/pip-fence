...
// Run: node heritage/evals/latency_cost.mjs
import { readFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
import { IDS, payload, phrase, parseWordlist, PROVIDERS } from "../src/buddy.mjs";
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  // the eight hinted ids (correct is never hinted) x tiers; miscounted_seeds alternates the side of the landing
  const payloads = Array.from({ length: 20 }, (_, i) => payload({ id: IDS[1 + (i % 8)], tier: 1 + (i % 3),
    called: i % 2 ? 1 : 3, landed: 2, path: [0, 1, 2, 3], node: "4_single" }));
  const rows = [];
