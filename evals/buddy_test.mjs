...
check("request: url/model/max_tokens/temperature/headers", url === "https://api.anthropic.com/v1/messages"
  && body.model === "claude-haiku-4-5-20251001" && body.max_tokens === 120 && body.temperature === 0.6
...
// ---- Pip plans the next fence: code lists, the model picks, code checks ----
{
  ...
  check("candidates: packs stay closed until three gold; three gold still keeps her in Build while a fence there is not gold", !candidates(M, { id: "correct", node: "3x4_concrete" }).some(n => n.endsWith("_packs")) && candidates({ ...M, "3x4_concrete": 1 }, { id: "correct", node: "3x4_concrete" }).every(n => n.endsWith("_concrete")));
  const ALL = Object.fromEntries(["2x3", "3x3", "3x4", "4x3", "4x5", "2x5"].map(s => [s + "_concrete", 1]));
  check("candidates: the chapter all gold opens the next one; mid-Packs she is never sent back to Build", candidates(ALL, { id: "correct", node: "2x5_concrete" }).every(n => n.endsWith("_packs")) && candidates({ ...ALL, "2x3_packs": 0.5, "2x3_concrete": 0.5 }, { id: "correct", node: "2x3_packs" }).every(n => n.endsWith("_packs")));
