...
|---|---|---|---|
| 0:00–0:15 | **Cold open** | `/build`, clean slate. Header "3 parts. 4 planks in each part." Cart with twelve planks, goat outside, corn inside. | "This is Rung. A child is told three parts, four planks each, and given exactly twelve planks. The maths is the build. There is no question to answer." |
...
| 1:30–1:55 | **The AI layer** | `coach.mjs` line 12 (*"The correct answer is …"*) beside `buddy.mjs` payload: no integer, `"groups":"some"`. Then the live bubble with `?debug=1` showing `model`. | "The old coach was told the answer. The new one is never given a number. A model only phrases the hint, through a gate that cannot emit a digit or a number word. Twenty live hints: median under a second, a hundredth of a cent each, and the gate rejected four for the word *one*." [LATENCY-RESULTS] |
| 1:55–2:15 | **Red-team** | `REDTEAM-RESULTS.md` Update 5 table. | "An attacker given the whole payload recovers the total no better than guessing the commonest one. Given only the hint the child sees, worse. Same family as the coach, which is a limitation I publish rather than hide." [REDTEAM-RESULTS §UPDATE 5] |
| 2:15–2:35 | **The pilot** | Paper test sheet (`TRANSFER-TEST.md`), then the three case-study lines, whatever they are. | "Three children, before and after and two days later, with two control items that were never taught. I ran the tests myself, unblinded. n equals three, no p-value." Then the result **in the pre-registered words if it is a null**. |
| 2:35–2:50 | **The parent** | `/public/parent.html`: fences this week, the open misconception, one question to ask out loud. Phone shot. | "Parents control return, so the retention screen is for the parent: one thing that happened and one question to ask at dinner." |
| 2:50–3:00 | **Close** | README top, repo tree, `python evals/run_all.py` ALL PASS. | "The mechanic is Zombie Division's, from 2011, credited. The gap between this and what already ships is narrower than the pitch implies. Everything I just said is measured in the repo." |
