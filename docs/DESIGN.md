...
(Habgood and Ainsworth, ages 7 to 11). Everything else serves that.
...
|---|---|---|
| Hint | Rephrases a template for a child of six | No digits, no number words, a 500-word list. A second model judges that the meaning is the same. Template on any failure |
...
- Every word she reads was checked against a 500-word early-reader list. Pip's fixed lines are
...
over its 162 sequences, and 57 checks on the hint layer. With a live key, the scripts in `evals/`
